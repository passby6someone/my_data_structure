...
Status InitStack(Stack<ElemType> &s){
    ...
    for (int i = 1; i < STACK_INIT_SIZE; i++)
    {
        ...
        node = (StackNode<ElemType>*)malloc(sizeof(StackNode<ElemType>));
        preNode -> next = node;
        preNode = node;
    }
    preNode -> next = NULL;
...
Status Push (Stack<ElemType> &s,ElemType e){
    if(s.top != NULL){
        ...
        StackNode<ElemType> *node = new StackNode<ElemType>,*preNode = node;
        s.top = node;
        for (int i = 1; i < STACKINCREMENT; i++)
        {
            node = new StackNode<ElemType>;
            preNode -> next = node;
            preNode = node;
...
//二叉树的后序遍历,index从0开始
template<typename ElemType>
int index(Stack<ElemType> S,ElemType e){
    StackNode<ElemType> *p = S.base;
    int i = 0;
    while (p != S.top && p->data != e)
    {
        p = p->next;
        i++;
    }
